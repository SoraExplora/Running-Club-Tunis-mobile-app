...
class _EventsScreenState extends State<EventsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final textSub = isDark ? Colors.white.withValues(alpha: 0.70) : Colors.black.withValues(alpha: 0.60);
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Icon(Icons.calendar_month_rounded, color: Theme.of(context).colorScheme.primary.withValues(alpha: 0.9)),
                  ],
                ),
              ),
  ...
  void _showQuitGroupDialog(BuildContext context, String userId) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text("Are you sure you want to leave your current group?"),
        actions: [
          ...
          TextButton(
            onPressed: () async {
              Navigator.pop(ctx);
              try {
                await Provider.of<GroupService>(context, listen: false).leaveGroup(userId);
                if (context.mounted) {
    ...
    return SizedBox(
      ...
      child: ListView.separated(
        ...
        itemBuilder: (_, i) {
          ...
          final bg = isSelected
              ? Theme.of(context).colorScheme.primary.withValues(alpha: isDark ? 0.22 : 0.18)
              : (isDark
          ...
          final text = isSelected
              ...
              : (isDark ? Colors.white.withValues(alpha: 0.78) : Colors.black.withValues(alpha: 0.72));
                  ...
                  Wrap(
                    ...
                    children: [
                      _MetaChip(
                        ...
                        text: event.group == 'All' ? "Everyone" : "Group ${event.group}",
                        isDark: isDark,
                      ),
                      _MetaChip(
...
class _TimePill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: _EventCard._kindColor(context, kind).withValues(alpha: isDark ? 0.22 : 0.16),
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: border),
      ),
      child: Column(
...
class _MetaChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final fg = isDark ? Colors.white.withValues(alpha: 0.78) : Colors.black.withValues(alpha: 0.72);
    final border = isDark ? Colors.white.withValues(alpha: 0.10) : Colors.black.withValues(alpha: 0.06);
...
class _EventDetailsSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final textMain = isDark ? Colors.white : Colors.black;
    final textSub = isDark ? Colors.white.withValues(alpha: 0.72) : Colors.black.withValues(alpha: 0.62);
      ...
      builder: (context, controller) {
        ...
        return StatefulBuilder(
          builder: (context, setSheetState) {
            ...
            final isParticipating = user != null && event.participants.contains(user.id);
            ...
            return Container(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: Text(
                          event.title,
                          style: TextStyle(fontWeight: FontWeight.w900, fontSize: 22, color: textMain),
                        ),
                      ),
                      if (!event.isFree)
                        Container(
                  ...
                  Text(
                    event.description,
                    style: TextStyle(color: textSub, fontWeight: FontWeight.w600, height: 1.4, fontSize: 15),
                  ),
                  const SizedBox(height: 20),

                  _detailRow(context, Icons.calendar_month_rounded, _fullDate(event.date), textMain, textSub),
                  const SizedBox(height: 12),
                  _detailRow(context, Icons.location_on_outlined, event.location, textMain, textSub),
                  const SizedBox(height: 12),
                  _detailRow(context, Icons.groups_2_rounded, event.group == 'All' ? "Everyone" : "Group ${event.group}", textMain, textSub),
                  const SizedBox(height: 12),
                  _detailRow(context, Icons.info_outline, _EventCard._kindLabel(event.kind), textMain, textSub),
                  const SizedBox(height: 12),
                  _detailRow(context, Icons.people_alt_rounded, "${event.participants.length} participants", textMain, textSub),
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed: isLoading ? null : () async {
                          ...
                          try {
                            await eventService.toggleParticipation(event.id, user.id);
                        ...
                        style: ElevatedButton.styleFrom(
                          backgroundColor: isParticipating ? Colors.red.withValues(alpha: 0.1) : theme.colorScheme.primary,
                          foregroundColor: isParticipating ? Colors.red : Colors.white,
                          elevation: 0,
                          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
                          side: isParticipating ? const BorderSide(color: Colors.red) : null,
                        ),
                        child: isLoading 
                          ? const SizedBox(width: 24, height: 24, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
                          : Text(
                              isParticipating ? "Cancel Participation" : "Join Session",
                  ...
                  Text("Participants", style: TextStyle(color: textMain, fontWeight: FontWeight.w900, fontSize: 16)),
                  const SizedBox(height: 12),
                  if (event.participants.isEmpty)
                    Text("No one has joined yet. Be the first!", style: TextStyle(color: textSub, fontStyle: FontStyle.italic))
                  else
                    Wrap(
                      spacing: 8,
                      runSpacing: 8,
                      children: event.participants.take(20).map((p) => Chip(
                        label: Text(p, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 13)),
                        backgroundColor: theme.colorScheme.primary.withValues(alpha: isDark ? 0.15 : 0.10),
  ...
  Widget _detailRow(BuildContext context, IconData icon, String value, Color textMain, Color textSub) {
    return Row(
      children: [
        Container(
        ...
        Expanded(
          child: Text(
            value,
            style: TextStyle(color: textSub, fontWeight: FontWeight.w700, fontSize: 14),
          ),
        ),
      ],
    );
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(Icons.hourglass_empty_rounded, size: 64, color: theme.colorScheme.primary),
            const SizedBox(height: 24),
            const Text(
              "Request Pending",
            ...
            Text(
              ...
              textAlign: TextAlign.center,
              style: TextStyle(color: textSub, fontWeight: FontWeight.w600, height: 1.4),
            ),
            const SizedBox(height: 32),
            OutlinedButton(
              onPressed: () => Provider.of<GroupService>(context, listen: false).denyJoinRequest(user.id),
