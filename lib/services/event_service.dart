...
class EventService with ChangeNotifier {
  ...
  final Map<String, Stream<List<EventModel>>> _eventStreams = {};

  // Get events for a specific group (or 'All')
  Stream<List<EventModel>> getEventsForGroup(String groupId) {
    if (_eventStreams.containsKey(groupId)) return _eventStreams[groupId]!;

    final stream = _firestore
        .collection('events')
        .where('group', whereIn: ['All', groupId])
        .snapshots()
        .map((snapshot) {
